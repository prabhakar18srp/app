...
import { Sparkles, Users, TrendingUp, MessageSquare, ArrowLeft } from 'lucide-react';
...
const CampaignDetailPage = () => {
  ...
  const fetchCampaignDetails = async () => {
    try {
      const [campaignResp, analysisResp, commentsResp] = await Promise.all([
        ...
        axiosInstance.get(`/campaigns/${id}/analysis`),
        axiosInstance.get(`/campaigns/${id}/comments`)
      ]);
  ...
  const handlePledge = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post(`/payments/create-checkout`, {
        campaign_id: id,
    ...
    <div className="min-h-screen py-12" data-testid="campaign-detail-page">
      <div className="container px-6 max-w-6xl">
        ...
        <button
          ...
          data-testid="back-to-campaigns-button"
        >
